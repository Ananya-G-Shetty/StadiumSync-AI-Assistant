...
export default function HomePage() {
  ...
  useEffect(() => {
    async function loadInitialData() {
      try {
        ...
        const defaultVenueId = savedVenue && vList.some((v: any) => v.id === savedVenue) 
          ? savedVenue 
          : (vList[0]?.id || '');
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <section className="space-y-3" aria-labelledby="matches-list-heading">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => router.push('/schedule')} className="text-[10px] font-black text-primary uppercase hover:underline">
                View All
              </button>
            </div>
                          ...
                          <td className="p-3 text-center whitespace-nowrap">
                            {isLive ? (
                              <span className="inline-flex bg-primary/15 border border-primary/30 text-primary text-[8px] font-black px-2 py-0.5 rounded-full items-center gap-1 animate-pulse">
                                <span className="h-1 w-1 bg-primary rounded-full" /> LIVE
                              </span>
                            ) : m.status === 'completed' ? (
      ...
      {/* MODAL 1: Operational Issue Reporter Dialog */}
      {isReportModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 backdrop-blur-sm p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md bg-card border border-border rounded-2xl overflow-hidden shadow-2xl p-5 space-y-4 animate-scale-up">
            ...
            <div className="flex justify-between items-center border-b border-border/60 pb-3">
              ...
              <button 
                onClick={() => { setIsReportModalOpen(false); setFormMessage(null); }}
